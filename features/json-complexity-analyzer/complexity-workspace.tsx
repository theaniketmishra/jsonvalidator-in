"use client";
import { ConverterWorkspace } from "@/components/tools/converter-workspace";
import { createJsonStore } from "@/hooks/create-json-store";
import { analyzeComplexity } from "@/lib/json/complexity";

const useStore = createJsonStore();

export function ComplexityWorkspace({ sample }: { sample: string }) {
  return (
    <ConverterWorkspace
      useStore={useStore}
      sample={sample}
      convert={analyzeComplexity}
      outputLanguage="plaintext"
      outputLabel="Analysis"
      downloadFilename="complexity-report.txt"
    />
  );
}
